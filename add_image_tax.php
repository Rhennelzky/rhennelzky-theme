...
if( ! class_exists( 'Showcase_Taxonomy_Images' ) ) {
  class Showcase_Taxonomy_Images {
     ...
     */
     ...
     // Image actions
     add_action( 'theme_add_form_fields', array( $this, 'add_category_image' ), 10, 2 );
     add_action( 'created_theme', array( $this, 'save_category_image' ), 10, 2 );
     add_action( 'theme_edit_form_fields', array( $this, 'update_category_image' ), 10, 2 );
     add_action( 'edited_theme', array( $this, 'updated_category_image' ), 10, 2 );
     add_action( 'admin_enqueue_scripts', array( $this, 'load_media' ) );
     add_action( 'admin_footer', array( $this, 'add_script' ) );
   }

   public function load_media() {
     if( ! isset( $_GET['taxonomy'] ) || $_GET['taxonomy'] != 'theme' ) {
       return;
     }
     wp_enqueue_media();
   }
...
add_filter("manage_edit-theme_columns", 'theme_columns'); 

function theme_columns_content( $content, $column_name, $term_id )
{ 
    $image_id = get_term_meta( $term_id, 'showcase-taxonomy-image-id', true );     
    if ( 'custom_media_image' == $column_name ) {
        $content = wp_get_attachment_image( $image_id, 'thumbnail' );
    }
  return $content;
}
add_filter( 'manage_theme_custom_column', 'theme_columns_content', 10, 3 );
